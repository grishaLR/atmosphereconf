...
function RichText({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <span className={className}>
      {segments.map((seg, i) =>
        seg.kind === "link" ? (
          <a key={i} href={seg.href} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
            {seg.text}
          </a>
        ) : seg.kind === "br" ? (
          <br key={i} />
...
function GermButton({ href }: { href: string }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-2 rounded-full border border-gray-200 px-1.5 py-1 text-sm font-semibold text-gray-700 hover:bg-gray-200/70 bg-gray-100"
    >
...
export function ProfileView({
  ...
  const [editDisplayName, setEditDisplayName] = useState(editData?.initialDisplayName ?? displayName);
  const [editDescription, setEditDescription] = useState(editData?.initialDescription ?? description ?? "");
  const [editBio, setEditBio] = useState(editData?.initialBio ?? bio ?? "");
  const [editInterests, setEditInterests] = useState(editData?.initialInterests ?? (interests?.join(", ") ?? ""));
  const [editPronouns, setEditPronouns] = useState(editData?.initialPronouns ?? pronouns ?? "");
  const [editWebsite, setEditWebsite] = useState(editData?.initialWebsite ?? website ?? "");
  const [location, setLocation] = useState<LocationData | null>(
    editData?.initialLocation
      ? { name: editData.initialLocation.name, lat: 0, lon: 0, h3Index: editData.initialLocation.h3Index }
      : null
  );
  ...
  async function handleSave() {
    ...
    try {
      let avatarBlob: BlobRef | undefined = editData.existingAvatarBlob ?? undefined;
      if (avatarFile) {
      ...
      const { error: profileError } = await actions.saveProfile({
        ...
        homeTown: location ? { name: location.name, value: location.h3Index } : undefined,
        avatar: avatarBlob,
      });
    ...
    <div className="flex flex-col gap-0">
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            <input
              ...
              className="ui-input text-xl font-bold w-full"
            />
          ) : (
          ...
          {isOwnProfile && !isEditing && (
            <>
              <button
                ...
                className="inline-flex items-center justify-center rounded-md text-sm font-semibold transition-all size-9 hover:bg-accent hover:text-accent-foreground"
              ...
              <form method="POST" action="/oauth/logout">
                <button
                  type="submit"
                  className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-semibold transition-all h-9 px-4 bg-secondary text-secondary-foreground hover:bg-secondary/80"
                >
                  Logout
                </button>
              </form>
          ...
          {isOwnProfile && isEditing && (
            <>
              <button
                onClick={handleSave}
                disabled={saving}
                className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-semibold transition-all h-9 px-4 bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
              >
                {saving ? "Saving..." : "Save"}
              </button>
              <button
              ...
              {isEditing ? (
                <>
                  <input
                    ...
                    maxLength={64}
                    className="ui-input text-xs w-40"
                  />
                  <input
                    value={editWebsite}
                ...
                <>
                  ...
                  {website && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-xs font-medium border border-gray-200 rounded-full px-2.5 py-0.5 text-gray-600 hover:bg-gray-50 truncate max-w-45"
                    >
      ...
      {showAbout && (
        <div className="mt-2 mb-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">About</div>
          {isEditing ? (
            <textarea
            ...
            <RichText text={description} className="text-gray-900 mt-0.5 block" />
          ) : null}
        </div>
      )}
      ...
      {showBio && (
        <div className="mb-3">
          ...
          {isEditing ? (
            ...
            <RichText text={bio} className="text-gray-900 mt-0.5 block whitespace-pre-wrap" />
          ) : null}
        </div>
      )}
      ...
      {showLocation && (
        <div className="mb-3">
          ...
          {isEditing ? (
            ...
            <div className="text-gray-900 mt-0.5">{homeTown?.name}</div>
          )}
        </div>
      )}
      ...
      {showInterests && (
        <div className="mb-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">Interests</div>
          {isEditing ? (
            <>
              <input
                value={editInterests}
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {interests.map((interest) => (
                <span key={interest} className="inline-flex items-center rounded-full border border-transparent bg-secondary text-secondary-foreground text-xs font-semibold px-2.5 py-0.5">
                  {interest}
                </span>
              ))}
            </div>
      ...
      {activeIcons.length > 0 && (
        <div className="mb-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">Active On</div>
          <div className="flex flex-wrap gap-3">
            {activeIcons.map((icon) => {
              ...
              return icon.href ? (
                <a key={icon.label} href={icon.href} target="_blank" rel="noopener noreferrer" title={icon.label} className="size-5">
                  {el}
                </a>
              ) : (
